...
def dashboard():
    ...
    # Retrieve the budget information from the database
    budget = Budget.query.filter_by(
        user_id=current_user.id).order_by(Budget.id.desc()).first()
    if budget:
        budget_limit = budget.budget_limit
    ...
    return render_template('dashboard.html', user=current_user, records=records_dict, budget_limit=budget_limit, total_spent=total_spent, category_budgets=category_budgets)
...
def transactions():
    ...
    if request.method == 'POST':
        ...
        total_amount = sum(record['amount'] for record in records_dict if start_date <= datetime.strptime(
            record['date'], '%Y-%m-%d') <= end_date)
